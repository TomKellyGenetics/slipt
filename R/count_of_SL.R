...
function(x, significance = "fdr"){
  if(significance == "holm"){
    sum((x[,7]==1) & (as.numeric(x[,9])<0.05)) #SL interactions & Holm Sig
  } else if(significance == "fdr"){
    sum((x[,7]==1) & (as.numeric(x[,10])<0.05)) #SL interactions & FDR Sig
  } else if(significance == "raw"){
    sum((x[,7]==1) & (as.numeric(x[,8])<0.05)) #SL interactions & raw Sig
    ...
    sum((x[,7]==1) & (p.adjust(as.numeric(x[,8]), method = significance)<0.05))#SL interactions & custom Sig adjust
  } else {
