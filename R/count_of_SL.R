...
function(x, significance = "fdr", threshold = 0.05, syn_lethal = "strong"){
  ...
  if(significance == "holm"){
    sum((x[,11] %in% syn_lethal_condition) & (as.numeric(x[,16])<threshold)) #SL interactions & Holm Sig
  } else if(significance == "fdr"){
    sum((x[,11] %in% syn_lethal_condition) & (as.numeric(x[,17])<threshold)) #SL interactions & FDR Sig
  } else if(significance == "raw"){
    sum((x[,11] %in% syn_lethal_condition) & (as.numeric(x[,15])<threshold)) #SL interactions & raw Sig
  } else if(significance == "none" | is.null(significance) | missing(significance) | is.na(significance)){
    sum(x[,11] %in% syn_lethal_condition) #SL interactions without signficance
  } else if(significance %in% p.adjust.methods){
    sum((x[,11] %in% syn_lethal_condition) & (p.adjust(as.numeric(x[,15]), method = significance)<threshold))#SL interactions & custom Sig adjust
  } else {
