...
function(x, significance = "fdr", threshold = 0.05){
  if(significance == "holm"){
    sum((x[,7]==1) & (as.numeric(x[,9])<threshold)) #SL interactions & Holm Sig
  } else if(significance == "fdr"){
    sum((x[,7]==1) & (as.numeric(x[,10])<threshold)) #SL interactions & FDR Sig
  } else if(significance == "raw"){
    sum((x[,7]==1) & (as.numeric(x[,8])<threshold)) #SL interactions & raw Sig
  } else if(significance == "none" | is.null(significance) | missing(significance) | is.na(significance)){
    sum(x[,7]==1) #SL interactions without signficance
  } else if(significance %in% p.adjust.methods){
    sum((x[,7]==1) & (p.adjust(as.numeric(x[,8]), method = significance)<threshold))#SL interactions & custom Sig adjust
  } else {
